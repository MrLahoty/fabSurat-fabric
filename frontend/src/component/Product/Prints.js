...
const Prints = ({ match }) => {
  ...
  useEffect(() => {
    ...
    // Fetch Prints products, with optional sub-category filter
    dispatch(
      getProduct(
        keyword,
        currentPage,
        committedPrice,
        "Fabric",
        "Prints", // The main category is set to Embroidered
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <MetaData title="Prints PRODUCTS -- FABSURAT" />
          <div className="productss">
            <h2 className="productsHeadingsssss">Prints</h2>
            ...
            <div className="subCategory-container">
              ...
              {/* Sub-category List */}
              <ul className={`categoryBoxsssss ${subCategoriesVisible ? "show" : "hide"}`}>
                {printsSubSubCategories.map((subCategory) => (
                  <li
